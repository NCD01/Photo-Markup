...
- Owner: NCD / M
- Area: Data Safety
- Decision: Unsaved markup is autosaved to one recovery file and offered back on the next launch. The autosave is debounced, not periodic: the timer starts when the markup goes from saved to unsaved and writes when it fires, so a long scribble produces one write when the hand stops rather than one per stroke. The wait is a Settings control, `Autosave after`, defaulting to 15 seconds with a range of 5 to 120, which is the control deliberately left out of v0.37 for want of an autosave to point at. A write also happens immediately when the app is backgrounded, hidden or asked to exit. The file lives at `%APPDATA%\NCD Photo Markup\recovery.ncdmarkup.json`, beside settings.json and never beside the photo, and is written to a `.partial` name then renamed so an interrupted write leaves the previous good copy. It is the same JSON as a user-saved markup file, so recovery is an ordinary open. On launch, a recovery file that parses, names a photo still on disk, holds at least one mark and is under seven days old is offered by photo name and mark count. Restoring reopens the photo, applies the document and leaves the work flagged unsaved. Declining deletes the file immediately. Saving or exporting deletes it. A file that fails any of the offer conditions is deleted where it is found rather than left to prompt forever. A photo handed in on the command line is never interrupted by the prompt.
- Alternatives Considered:
- A periodic timer writing every N seconds regardless of activity. Rejected: it writes constantly while nothing changes and still misses the stroke drawn a moment before a crash.
- Autosaving next to the photo, as a sidecar. Rejected: it puts app-generated files into the customer's job folder, and the photo folder is often a network share or a phone import that is not writable.
- Restoring silently with no prompt. Rejected: silently reopening a photo the user has moved on from is worse than asking, and a restore that is not asked for cannot be declined.
- Keeping a declined recovery file in case the user changes their mind. Rejected: it prompts on every launch forever, and a prompt already answered is noise.
- Rationale: This is the only item in the queue that protects something that cannot be recreated. A tool that loses an afternoon of markup because a laptop lid closed is not trusted again. The debounced design was chosen because the failure mode that matters is a crash during or just after drawing, and a debounce fires exactly then, whereas a periodic timer is most likely to be mid-interval. Hooking the autosave to the single `UnsavedChangesTracker.onChanged` callback rather than to each of the fifteen markDirty call sites means a markup type added later cannot forget to be recoverable.
- Impact: The source photo is never opened for writing anywhere in this path, and a test asserts its bytes and modification time are unchanged across an autosave. Twenty tests: fourteen on the service, six driving the real widget tree, covering the write, the interrupted write, the offer, the decline, the Control Center case and the interval setting. Adds no dependency; built on dart:io like the settings service beside it. Every failure path is swallowed, because an exception on a background timer taking down the app the user is drawing in would be worse than a missed autosave.
